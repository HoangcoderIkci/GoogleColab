...
__global__ void supportF2(U8* F, const U64 trust_length_block)
{
...
{
    cudaError_t cudaStatus;
    cudaStatus = cudaMallocManaged(&global_table_GPU, glo_length * sizeof(U8));
    if (cudaStatus == cudaSuccess) {
        printf("Cấp phát động thành công cho global_table.\n");
    }
    else {
    ...
    cudaMemset(global_table_GPU, 0, glo_length * sizeof(U8));
    return 1;
}
...
void fastFindCoefficientsRealCPU(int8_t* lst_coefficients)
{
    U64 half_len;
    U64 full_len;
    U64 num_turns;
...
int main()
{
   ...
   */
   //  giai doan 2
    ...
    start = clock();
    fastFindCoefficientsZhegalkinGPU(global_table_GPU);
    //printf("finish GPU\n");
    //fastFindCoefficientsZhegalkinCPU(global_table_CPU);
    end = clock();
    cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC;
