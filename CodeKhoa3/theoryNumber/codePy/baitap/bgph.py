...
def getRemainderDivider(F_x,G_x,w,MOD):
    ...
    for i in range(1,k+1):
        bac_hien_tai = (1 << i)
        P_x = PhiSupport(chuan_hoa_he_so(P_x[:bac_hien_tai]),chuan_hoa_he_so(G_x_inv[:bac_hien_tai]),w,module,bac_hien_tai)
        #(P_x)
        P_x = P_x[:(1 << i)]
        P_x = chuan_hoa_he_so(P_x)
        printfPoly(P_x,module)
        #print(P_x)
    # da co H_x = P_x
    #b4 tinh Q_x = P_x * F_x_inv
    #P_x = P_x[:(1 << k)]
    #check h_x * g_x_inv == 1 ?
    #h_x_g_inv = multipleTwoPolynomialverSupper(P_x,G_x_inv,w,module)
    #print(h_x_g_inv)
    
    
    Q_x = multipleTwoPolynomialverSupper(chuan_hoa_he_so(F_x_inv[:deg_F-m+1]),chuan_hoa_he_so(P_x[:deg_F-m+1]),w,module)
    printfPoly(Q_x,module)
...
if __name__ == '__main__':
    ...
    #Q_x = [0, 0, 1, 0, 0, 1]
    # product = multiply_polynomials_check(dividend,quotient,mod)
    # print(quotient,remainder)
    # print(product)
    # print(multipleTwoPolynomialverSupper(F,G,w,3))
