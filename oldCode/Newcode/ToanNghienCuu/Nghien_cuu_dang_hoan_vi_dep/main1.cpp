...
uint32_t arrAddition[SIZEPOLE][SIZEPOLE];
uint32_t arrInverseSubtraction[SIZEPOLE];
uint32_t arrOppositeArray[SIZEPOLE];
...
Matrix transpose(const Matrix &A)
{
    ...
    Matrix B(cols, vector<uint32_t>(rows)); // Chú ý rằng số hàng và cột đã được đảo ngược
...
Matrix addMatrices(const Matrix &A, const Matrix &B)
{
    ...
    Matrix C(rows, vector<uint32_t>(cols, 0));
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
...
Matrix multiplyMatrices(const Matrix &A, const Matrix &B)
{
    ...
    Matrix C(r1, vector<uint32_t>(c2, 0));
    uint32_t temp = 0;
    for (int i = 0; i < r1; i++)
    {
        for (int j = 0; j < c2; j++)
